...
const TechText = ({
  ...
  useEffect(() => {
    ...
    const sprite = (s, view, glyph, stroke) => {
      ...
      if (stroke) {
        c.lineJoin = 'round'; c.lineWidth = s.strokeWidth * 2; c.lineCap = 'butt';
    ...
    const ensureLayout = s => {
      const key = [s.text, family(s), s.fontWeight, s.fontSize, s.letterSpacing, s.color,
        s.dashLength, s.dashGap, s.strokeWidth, s.lineStyle, width, height, dpr].join('|');
      ...
      const next = { size, baseline, left: x - m.actualBoundingBoxLeft, right: x + m.actualBoundingBoxRight,
      ...
      chars.forEach((char, i) => {
        ...
        glyphs.push({ ...base, offset: kept?.char === char ? kept.offset : { x: 0, y: 0 },
          velocity: { x: 0, y: 0 }, outline: 0, index: i,
          fill: sprite(s, next, base, false), dashes: sprite(s, next, base, true) });
      });
    ...
    const glyphAt = (x, y) => {
      ...
      glyphs.forEach((glyph, i) => {
        const x1 = glyph.box.x1 + glyph.offset.x, x2 = glyph.box.x2 + glyph.offset.x;
    ...
    const drawFrame = s => {
      ...
      ctx.lineWidth = 1; ctx.strokeStyle = rgba(s.accentColor, 0.5 * a); ctx.stroke();
      ctx.beginPath();
    ...
    const tick = now => {
      ...
      if (focus >= 0 && s.selection) {
        ...
        const bx1 = glyph.box.x1 + glyph.offset.x - 6, by1 = glyph.box.y1 + glyph.offset.y - 6;
        const bx2 = glyph.box.x2 + glyph.offset.x + 6, by2 = glyph.box.y2 + glyph.offset.y + 6;
      ...
      for (const glyph of glyphs) {
        const moved = Math.hypot(glyph.offset.x, glyph.offset.y);
        if (moved > 1) { ctx.globalAlpha = Math.min(1, moved / 24) * 0.55; blit(ctx, glyph.dashes, 0, 0, 0, 0); ctx.globalAlpha = 1; }
      }
      for (const glyph of glyphs) {
      ...
      drawFrame(s);
      const settling = moving || Math.abs(presence - (s.reveal === 'area' && active && dragging < 0 ? 1 : 0)) > 0.002
    ...
    const onDown = e => {
      ...
      if (s?.draggable && (e.pointerType !== 'mouse' || e.button === 0)) {
        ...
        if (index >= 0) { dragging = index; grab.x = pointer.x - glyphs[index].offset.x; grab.y = pointer.y - glyphs[index].offset.y; container.setPointerCapture?.(e.pointerId); }
      }
      wake();
    };
    const onUp = e => {
      if (dragging >= 0) { dragging = -1; container.releasePointerCapture?.(e.pointerId); const rect = container.getBoundingClientRect(); pointer.inside = e.clientX >= rect.left && e.clientX <= rect.right && e.clientY >= rect.top && e.clientY <= rect.bottom; }
      wake();
    };
    ...
    return () => {
      ...
      container.removeEventListener('pointermove', onMove); container.removeEventListener('pointerenter', onMove);
      container.removeEventListener('pointerdown', onDown); container.removeEventListener('pointerup', onUp);
      container.removeEventListener('pointercancel', onUp); container.removeEventListener('pointerleave', onLeave);
    };
  }, []);
